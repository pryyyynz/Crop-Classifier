...
from train_tomato import EfficientNetClassifier, get_transforms
import logging
...
class TomatoRawDataset(Dataset):
    def __init__(self, data_dir, transform=None):
        ...
        self.classes = tree_data['Combined']['Augmented']['Tomato']['train_set']
        self.class_to_idx = {cls: idx for idx, cls in enumerate(self.classes)}

        # Build file paths from raw data
        self.samples = []
        raw_path = self.data_dir / 'Combined' / 'Raw' / 'CCMT' / 'Tomato'
                ...
                for img_path in class_dir.glob('*'):
                    if img_path.suffix.lower() in ['.jpg', '.jpeg', '.png']:
                        ...
                        try:
                            ...
                            logger.warning(f"Skipping corrupted image: {img_path} - Error: {e}")

        print(f"Found {len(self.samples)} valid images in raw tomato dataset")
        if corrupted_files:
        ...
        print("\nClass distribution in raw tomato data:")
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            if self.transform:
                # Create a black image with the expected size
...
def plot_detailed_results(y_true, y_pred, class_names, save_dir):
    ...
    plt.title('Confusion Matrix - Tomato Raw Data Test Results', fontsize=16)
    plt.ylabel('True Label', fontsize=12)
    plt.xlabel('Predicted Label', fontsize=12)
    plt.xticks(rotation=45, ha='right')
    plt.tight_layout()
    plt.savefig(save_dir / 'tomato_raw_confusion_matrix.png', dpi=300)
    plt.close()
    ...
    plt.title('Per-Class Accuracy on Raw Tomato Data', fontsize=16)
    plt.xlabel('Disease Class', fontsize=12)
    plt.ylabel('Accuracy (%)', fontsize=12)
    plt.xticks(rotation=45, ha='right')
    plt.ylim(0, 105)
    plt.tight_layout()
    plt.savefig(save_dir / 'tomato_raw_per_class_accuracy.png', dpi=300)
    plt.close()
...
def main():
    ...
    model_path = models_dir / 'best_tomato_model.pth'
    ...
    raw_dataset = TomatoRawDataset(data_dir, transform=test_transform)
    ...
    # Print results
    print(f"\n{'='*60}")
    print(f"TOMATO TEST RESULTS ON RAW DATA")
    print(f"{'='*60}")
    ...
    with open(results_dir / 'tomato_raw_classification_report.txt', 'w') as f:
        f.write(f"TOMATO TEST RESULTS ON RAW DATA\n")
        f.write(f"{'='*60}\n")
