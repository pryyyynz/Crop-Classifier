...
from train_cassava import EfficientNetClassifier, get_transforms
...
def main():
    ...
    if misclassified:
        ...
        for i, miss in enumerate(misclassified[:10]):
            ...
            print(f"   True: {miss['true_label']} | Predicted: {miss['predicted_label']} (Confidence: {miss['confidence']:.2f})")
    ...
    correct_confidences = [conf for conf, pred, true in zip(confidence_scores, predictions, targets) if pred == true]
    incorrect_confidences = [conf for conf, pred, true in zip(confidence_scores, predictions, targets) if pred != true]
