from training.train_cashew import EfficientNetClassifier, get_transforms
import os
import json
import torch
...
class CashewRawDataset(Dataset):
    def __init__(self, data_dir, transform=None):
        ...
        self.classes = tree_data['Combined']['Augmented']['Cashew']['train_set']
        self.class_to_idx = {cls: idx for idx, cls in enumerate(self.classes)}

        # Build file paths from raw data
        self.samples = []
        raw_path = self.data_dir / 'Combined' / 'Raw' / 'CCMT' / 'Cashew'
        ...
        print(f"Found {len(self.samples)} images in raw cashew dataset")
        ...
        print("\nClass distribution in raw cashew data:")
...
def plot_detailed_results(y_true, y_pred, class_names, save_dir):
    ...
    plt.title('Confusion Matrix - Cashew Raw Data Test Results', fontsize=16)
    plt.ylabel('True Label', fontsize=12)
    plt.xlabel('Predicted Label', fontsize=12)
    plt.xticks(rotation=45, ha='right')
    plt.tight_layout()
    plt.savefig(save_dir / 'cashew_raw_confusion_matrix.png', dpi=300)
    plt.close()
    ...
    plt.title('Per-Class Accuracy on Raw Cashew Data', fontsize=16)
    plt.xlabel('Disease Class', fontsize=12)
    plt.ylabel('Accuracy (%)', fontsize=12)
    plt.xticks(rotation=45, ha='right')
    plt.ylim(0, 105)
    plt.tight_layout()
    plt.savefig(save_dir / 'cashew_raw_per_class_accuracy.png', dpi=300)
    plt.close()
...
def main():
    ...
    with open(data_dir / 'tree.json', 'r') as f:
        tree_data = json.load(f)
    class_names = tree_data['Combined']['Augmented']['Cashew']['train_set']
    print(f"\nClasses: {class_names}")

    # Load model
    model = EfficientNetClassifier(num_classes=5, model_name='efficientnet_b1')
    model_path = models_dir / 'best_cashew_model.pth'
    ...
    raw_dataset = CashewRawDataset(data_dir, transform=test_transform)
    ...
    # Print results
    print(f"\n{'='*60}")
    print(f"CASHEW TEST RESULTS ON RAW DATA")
    print(f"{'='*60}")
    ...
    with open(results_dir / 'cashew_raw_classification_report.txt', 'w') as f:
        f.write(f"CASHEW TEST RESULTS ON RAW DATA\n")
        f.write(f"{'='*60}\n")
