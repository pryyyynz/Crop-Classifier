from training.train_maize import EfficientNetClassifier, get_transforms
import os
import json
import torch
...
class MaizeRawDataset(Dataset):
    def __init__(self, data_dir, transform=None):
        ...
        self.classes = tree_data['Combined']['Augmented']['Maize']['train_set']
        self.class_to_idx = {cls: idx for idx, cls in enumerate(self.classes)}

        # Build file paths from raw data and validate images
        self.samples = []
        raw_path = self.data_dir / 'Combined' / 'Raw' / 'CCMT' / 'Maize'
        corrupted_files = []
                ...
                for img_path in class_dir.glob('*'):
                    if img_path.suffix.lower() in ['.jpg', '.jpeg', '.png']:
                        ...
                        try:
                            ...
                            print(
                                f"Warning: Skipping corrupted image {img_path}: {e}")

        print(f"Found {len(self.samples)} valid images in raw maize dataset")
        if corrupted_files:
        ...
        print("\nClass distribution in raw maize data:")
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            fallback_image = Image.new('RGB', (240, 240), (0, 0, 0))
            if self.transform:
...
def plot_detailed_results(y_true, y_pred, class_names, save_dir):
    ...
    plt.title('Confusion Matrix - Maize Raw Data Test Results', fontsize=16)
    plt.ylabel('True Label', fontsize=12)
    plt.xlabel('Predicted Label', fontsize=12)
    plt.xticks(rotation=45, ha='right')
    plt.yticks(rotation=0)
    plt.tight_layout()
    plt.savefig(save_dir / 'maize_raw_confusion_matrix.png', dpi=300)
    plt.close()
    ...
    plt.title('Per-Class Accuracy on Raw Maize Data', fontsize=16)
    plt.xlabel('Disease Class', fontsize=12)
    plt.ylabel('Accuracy (%)', fontsize=12)
    plt.xticks(rotation=45, ha='right')
    plt.ylim(0, 105)
    plt.tight_layout()
    plt.savefig(save_dir / 'maize_raw_per_class_accuracy.png', dpi=300)
    plt.close()
...
def main():
    ...
    with open(data_dir / 'tree.json', 'r') as f:
        tree_data = json.load(f)
    class_names = tree_data['Combined']['Augmented']['Maize']['train_set']
    print(f"\nClasses: {class_names}")

    # Load model
    model = EfficientNetClassifier(
        num_classes=7, model_name='efficientnet_b1')  # Maize has 7 classes
    model_path = models_dir / 'best_maize_model.pth'
    ...
    raw_dataset = MaizeRawDataset(data_dir, transform=test_transform)
    ...
    # Print results
    print(f"\n{'='*60}")
    print(f"MAIZE TEST RESULTS ON RAW DATA")
    print(f"{'='*60}")
    ...
    with open(results_dir / 'maize_raw_classification_report.txt', 'w') as f:
        f.write(f"MAIZE TEST RESULTS ON RAW DATA\n")
        f.write(f"{'='*60}\n")
